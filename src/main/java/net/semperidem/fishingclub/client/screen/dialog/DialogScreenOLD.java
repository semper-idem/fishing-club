...
public class DialogScreenOLD extends Screen {
    Identifier TEXTURE = new Identifier(FishingClub.MOD_ID, "textures/gui/derek_dialog.png");
    Identifier PLAYER_ICON;
    ...
    public DialogScreenOLD(Text text) {
        super(text);
        this.client = MinecraftClient.getInstance();
        PLAYER_ICON = MinecraftClient.getInstance().player.getSkinTexture();
    }
    ...
    protected void init() {
        ...
        this.y = this.height - this.backgroundHeight;

        this.dialogBox = new DialogBoxWidget(client, 168,60,x + 66,y + 12);
...
//THIS RESETS DIALOG IT SHOULD NOT
        ...
        initialChoiceY = y + 78;
        DialogChoiceWidget dummyChoice = new DialogChoiceWidget(x + 66, initialChoiceY, null);
    ...
    public void renderBackground(MatrixStack matrixStack) {
        ...
        setTexture(TEXTURE);
        drawTexture(matrixStack, x, y, 0, 0, backgroundWidth, backgroundHeight, backgroundWidth, backgroundHeight);
    }
    ...
    public void render(MatrixStack matrixStack, int i, int j, float f) {
        ...
        setTexture(PLAYER_ICON);
        drawTexture(matrixStack, (int) ((x + 6) / scale), (int) ((height - 54) / scale), 32,32,32,32);
        drawTexture(matrixStack, (int) (x / scale), (int) (y / scale), 160,32,32,32);
        matrixStack.pop();
    ...
    class DialogBoxWidget extends AlwaysSelectedEntryListWidget<Entry> {
        int textSpeed = 1;
        int textTick = 0;
        DialogScreenOLD.Entry lastDialog;
        ...
        public DialogScreenOLD.Entry getDialogEntry(int i){
            return getEntry(i);
        }
        @Override
        ...
        void addMessage(DialogScreenOLD.Entry entry) {
            textTick = 0;
            addEntry(entry);
    ...
    class DialogChoiceWidget extends ButtonWidget {
        ...
        public void init(DialogScreenOLD.Entry initialEntry){
            this.option = initialEntry;
            this.populateChoices();
        }

        public DialogChoiceWidget getDefaultChoice(int x, int y,  DialogScreenOLD.Entry option){
        ...
        public DialogChoiceWidget getShopChoice(int x, int y, DialogScreenOLD.Entry option){
            DialogChoiceWidget choice = new DialogChoiceWidget(x,y, buttonWidget -> {
                if (!isActive()) return;
                onShopClick();
            });
        ...
        private void populateChoices(){
            ...
            for(int i = 0; i < option.choices.size(); i++) {
                DialogScreenOLD.Entry choiceEntry = option.choices.get(i);
        ...
        private DialogChoiceWidget getChoice(DialogScreenOLD.Entry choiceEntry, int i){
        ...
        public void render(MatrixStack matrixStack, int i, int j, float f) {
            ...
            textRenderer.drawWithShadow(matrixStack, (index + 1) + ") " + getMessage(), this.x, this.y + 1, Color.WHITE.getRGB());
        }
    }
}
