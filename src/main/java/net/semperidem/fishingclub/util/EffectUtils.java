...
public class EffectUtils {
    ...
    public static void onDerekSummonEffect(ServerWorld serverWorld, FishermanEntity fishermanEntity) {
        ...
        serverWorld.playSound(null, pos.x, pos.y, pos.z, SoundEvents.ENTITY_ENDERMAN_TELEPORT, SoundCategory.PLAYERS, 0.5f,1f, 0L);
        serverWorld.playSoundFromEntity(null, fishermanEntity, SoundEvents.ITEM_BUCKET_FILL_FISH, SoundCategory.PLAYERS, 1f, 1f, 0L);
    }

    public static void onDerekDisappearEffect(ServerWorld serverWorld, FishermanEntity fishermanEntity) {
        Vec3d deltaPosition = new Vec3d(1,1,1);
        Vec3d pos = fishermanEntity.getPos().add(0,1,0);
        smokeBombEffect(serverWorld, pos, deltaPosition);
        serverWorld.playSound(null, pos.x, pos.y, pos.z, SoundEvents.ENTITY_ENDERMAN_TELEPORT, SoundCategory.PLAYERS, 1f, 1f, 0L);
        serverWorld.playSound(null, pos.x, pos.y, pos.z, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.PLAYERS, 0.3f, 1f, 0L);
    }

    private static void smokeBombEffect(ServerWorld serverWorld, Vec3d position, Vec3d deltaPosition) {
