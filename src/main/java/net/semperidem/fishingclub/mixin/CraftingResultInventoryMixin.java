...
public class CraftingResultInventoryMixin implements RecipeUnlocker{
    ...
    public boolean shouldCraftRecipe(World world, ServerPlayerEntity player, Recipe<?> recipe){
        ...
        if (recipe.getOutput().isOf(FItemRegistry.FISHING_NET) && !FisherInfoManager.getFisher(player).hasPerk(FishingPerks.FISHING_NET)) return false;
        if (recipe.getOutput().isOf(FItemRegistry.DOUBLE_FISHING_NET) && !FisherInfoManager.getFisher(player).hasPerk(FishingPerks.UPGRADE_NET)) return false;
